...
def eval(testset, repeat, fold):
    ...
    Path(f'./result/sm2').mkdir(parents=True, exist_ok=True)
    record.to_csv(f'./result/sm2/repeat{repeat}_fold{fold}_{int(time.time())}.tsv', sep='\t', index=False)
