...
class SpacedRepetitionModel(object):
    ...
    def __init__(self, trainset, testset, method='HLR', omit_h_term=False, omit_lexemes=False, initial_weights=None,
                 lrate=.001, hlwt=.002,
                 l2wt=.001, sigma=1.):
        self.method = method
        self.omit_h_term = omit_h_term
        self.omit_lexemes = omit_lexemes
    ...
    def eval(self, repeat, fold, prefix=None):
        ...
        if prefix:
            ...
            sys.stderr.write(
                '%d\t%.1f (p=%.1f, h=%.1f, l2=%.1f)\tmae(p)=%.3f\tcor(p)=%.3f\tmae(h)=%.3f\tcor('
                'h)=%.3f\n' % \
                (self.cur_iter, total_loss, total_slp, self.hlwt * total_slh, self.l2wt * total_l2,
                 mae_p, cor_p, mae_h, cor_h))
        else:
            self.dump_detailed_predictions(
                f'./result/{self.method}{"-lex" if self.method == "HLR" and self.omit_lexemes else ""}/repeat{repeat}_fold{fold}_{int(time.time())}.tsv',
                self.testset)
    ...
    def dump_detailed_predictions(self, fname, testset):
        ...
        with open(fname, 'w') as f:
            f.write('r_history\tt_history\tp_history\tt\th\thh\tp\tpp\n')
            for inst in testset:
                pp, hh = self.predict(inst)
                f.write('%s\t%s\t%s\t%d\t%.4f\t%.4f\t%.4f\t%.4f\n' % (
                    inst.r_history, inst.t_history, inst.p_history, inst.t, inst.h, hh, inst.p, pp))
        result = pd.read_csv(fname, sep='\t', index_col=None)
        result['ae'] = round(abs(result['p'] - result['pp']), 3)
        result['ape'] = round(abs(result['hh'] - result['h']) / result['h'], 3)
        result.to_csv(fname, sep='\t', index=None)
